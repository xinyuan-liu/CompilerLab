...
public class MethodDeclaration implements Node {
   ...
   public List<FormalParameter> getFormalParameterList() {
	   if(f4.node==null)
		   return null;
	   FormalParameterList tmp=(FormalParameterList) f4.node;
	   ...
	   if(tmp.f1.present())
	   {
		   ...
		   for(int i=0;i<len;i++)
		   {
			   l.add(((FormalParameterRest)tmp.f1.elementAt(i)).f1);
		   }
	   }
	   return l;
   }
   
   public List<VarDeclaration> getLocalVarList() {
	   if(!f7.present())
		   return null;
	   ...
	   for(int i=0;i<len;i++)
	   {
		   l.add((VarDeclaration) f7.elementAt(i));
	   }
	   return l;
   }
